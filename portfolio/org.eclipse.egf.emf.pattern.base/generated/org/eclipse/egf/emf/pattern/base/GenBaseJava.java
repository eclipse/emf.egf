    ...
    protected void method_createImportManager(final StringBuffer stringBuffer, final PatternContext ctx) throws Exception {
        ...
        importManager = new ImportManager(packageName) {
            ...
            public void emitSortedImports() {
                ...
                Node.Leaf importNode = getImportNode((Node.Container) node.getContainer(Node.Container.class));
                if (importNode == null) {
            ...
            protected Node.Leaf getImportNode(final Container container) {
                for (Node node : container.getChildren())
                    if (node instanceof Container) {
                        Node.Leaf result = getImportNode((Container) node);
                        if (result != null)
                            return result;
                    } else if (node instanceof Node.Leaf) {
                        Node.Leaf result = (Node.Leaf) node;
