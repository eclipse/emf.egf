				...
				for (Map.Entry<GenClass, List<GenClass.ChildCreationData>> classEntry : packageEntry.getValue().entrySet()) {
					...
					for (GenClass.ChildCreationData childCreationData : classEntry.getValue()) {
						if (childCreationData.createClassifier instanceof GenClass
								&& (childCreationData.delegatedFeature == null || classEntry.getKey().getAllGenFeatures().contains(
							...
							new CodegenGeneratorAdapter(parameter).generateGIF("edit/CreateChild.gif", genClass.getCreateChildIconFileName(
									genModel, feature, childClass), genClass.getName(), childClass.getName(), false);
						}
					}
				}
			}
