...
public class InterfacegenFeatureoverride {
	...
	public static synchronized InterfacegenFeatureoverride create(
	...
	protected final String TEXT_25 = NL + "\t\tlist.shrink();" + NL
			+ "\t\treturn (";
	...
	protected final String TEXT_33 = "(int index)" + NL + "\t{" + NL
			+ "\t\treturn ";
	...
	protected final String TEXT_68 = "().set(index, element);" + NL + "\t}"
			+ NL;
	...
	protected void method_doGenerate(final StringBuffer stringBuffer,
		...
		if (genModel.isArrayAccessors() && genFeature.isListType()
				&& !genFeature.isFeatureMapType() && !genFeature.isMapType()) {
			...
			if (!isImplementation) {
				...
				if (genFeature.isVolatile()) {
					...
					stringBuffer.append(genFeature
							.getListTemplateArguments(genClass));
					...
					if (genModel.useGenerics()
							&& !genFeature.getListItemType(genClass).contains(
									"<")
							&& !genFeature.getListItemType(null).equals(
									genFeature.getListItemType(genClass))) {
		...
		if (genFeature.isGet()
				&& (isImplementation || !genFeature.isSuppressedGetVisibility())) {
		...
		if (genFeature.isSet()
				&& (isImplementation || !genFeature.isSuppressedSetVisibility())) {
