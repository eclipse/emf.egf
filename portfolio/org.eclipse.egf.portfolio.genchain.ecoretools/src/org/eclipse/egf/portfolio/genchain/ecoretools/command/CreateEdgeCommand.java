...
/**
 * Recording command allowing to create an Edge in a diagram.
 * 
...
public class CreateEdgeCommand extends RecordingCommand {
	...
	protected void doExecute() {
		...
		try {
			...
			RefreshIdsHolder holder = new RefreshIdsHolder();
			for (EdgeTarget sourceEdgeTarget : sourceNodes) 
			{
				for (EdgeTarget targetEdgeTarget : targetNodes) 
				{
					...
					if (precondition)
					{
						DEdgeCandidate candidate = new DEdgeCandidate(_mapping,_target, sourceEdgeTarget, targetEdgeTarget, holder);
			...
			//TODO: handle in a better way exceptions
			e.printStackTrace();
		}
	}
