    ...
    if (element == null) {
      ...
      if (element != null) {
        ...
        try {
          ...
          String msg = NLS.bind(
            "AbstractExtensionChangesCommand.getExtensionElementWithId(..) _ '{0}'", //$NON-NLS-1$
            ce.toString()
          );
  ...
  protected IPluginElement getElement(
    ...
    if (result == null && createIfMissing_p) {
      ...
      try {
        ...
        String msg = NLS.bind(
          "AbstractExtensionChangesCommand.getElement(..) _ '{0}'", //$NON-NLS-1$
          ce.toString()
        );
        EGFPDEPlugin.getDefault().log(msg, ce);
        if (EGFPDEPlugin.getDefault().isDebugging()) {
  ...
  protected boolean removeExtensionElementChild(
    ...
    if (extensionElements != null && matchingExtensionCount > 0) {
      ...
      for (int i = 0; i < matchingExtensionCount && !result; i++) {
        ...
        result = ExtensionHelper.removePluginElement(
          extensionElements[i], 
