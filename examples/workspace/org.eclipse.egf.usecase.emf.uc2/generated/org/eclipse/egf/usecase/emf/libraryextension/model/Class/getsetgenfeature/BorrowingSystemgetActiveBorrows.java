...
public class BorrowingSystemgetActiveBorrows
	...
	protected final String TEXT_1 = "\t\t//INJECTED-CODE-BEGIN : BorrowingSystemImpl#getActiveBorrows()" + NL
			+ "\t\tEList<RecordType> temp_records = getRecords();" + NL + "" + NL
			+ "\t\t// should be an unmodifiable list" + NL
			+ "\t\tBasicEList<Borrow> borrows = new BasicEList<Borrow>();" + NL
			+ "\t\tfor (RecordType recordType : temp_records) {" + NL + "\t\t\tif (recordType instanceof Borrow)" + NL
			+ "\t\t\t\tborrows.add((Borrow) recordType);" + NL + "\t\t}" + NL + "\t\treturn borrows;" + NL
			+ "\t\t//INJECTED-CODE-BEGIN : BorrowingSystemImpl#getActiveBorrows()" + NL;
