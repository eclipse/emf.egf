...
public class CrossLibraryReservationgetDescription extends org.eclipse.egf.emf.pattern.model.call.Class.Classinsert {
	...
	protected final String TEXT_1 = "\t//INJECTED-CODE-BEGIN : CrossLibraryReservationImpl#getDescription()" + NL
			+ "\t/**" + NL + "\t * <!-- begin-user-doc -->" + NL + "\t * <!-- end-user-doc -->" + NL + "\t * @generated"
			+ NL + "\t */" + NL + "\t@Override" + NL + "\tpublic String getDescription() {" + NL + "" + NL
			+ "\t\tString result = \"\";" + NL + "" + NL + "\t\tString descrFormat = getDescriptionFormat();" + NL
			+ "\t\tif (null != descrFormat)" + NL + "\t\t\tresult = result + descrFormat;" + NL + "" + NL
			+ "\t\tString items = \"\";" + NL + "\t\tEList<CirculatingItem> myItems = getItem();" + NL + "" + NL
			+ "\t\tfor (CirculatingItem circulatingItem : myItems) {" + NL + "\t\t\tif (items.length() > 0) {" + NL
			+ "\t\t\t\titems = items + (\";\");" + NL + "\t\t\t}" + NL + "\t\t\tif (circulatingItem instanceof Book)"
			+ NL + "\t\t\t\titems = items + (((Book) circulatingItem).getTitle());" + NL + "" + NL
			...
			+ "\t\tBorrower reservationBorrower = getBorrower();" + NL + "\t\tString borrowerName = \"\";" + NL + ""
			+ NL + "\t\tif (null != reservationBorrower)" + NL + "\t\t\tif (null != reservationBorrower.getFirstName())"
			+ NL + "\t\t\t\tif (null != reservationBorrower.getLastName())" + NL
			+ "\t\t\t\t\tborrowerName = reservationBorrower.getFirstName() + \" \"" + NL
			+ "\t\t\t\t\t\t\t+ reservationBorrower.getLastName();" + NL + "" + NL
			+ "\t\tString waitedBorrowerNames = \"\";" + NL + "\t\tString involvedLibrariesString = \"\";" + NL + ""
			+ NL + "\t\tfor (Borrower currentBorrower : getWaitedBorrowers()) {" + NL
			+ "\t\t\tif (waitedBorrowerNames.length() > 0) {" + NL + "\t\t\t\twaitedBorrowerNames += \";\";" + NL
			+ "\t\t\t}" + NL + "\t\t\twaitedBorrowerNames += currentBorrower.getFirstName() + \" \"" + NL
			+ "\t\t\t\t\t+ currentBorrower.getLastName();" + NL + "\t\t}" + NL + "" + NL
			+ "\t\tfor (EquippedLibrary currentLibrary : getInvolvedLibraries()) {" + NL
			+ "\t\t\tif (involvedLibrariesString.length() > 0) {" + NL + "\t\t\t\tinvolvedLibrariesString += \";\";"
			+ NL + "\t\t\t}" + NL + "\t\t\tinvolvedLibrariesString += currentLibrary.getName();" + NL + "\t\t}" + NL
			+ "" + NL + "\t\tif (isFinished())" + NL + "\t\t\tresult = result + \"[DONE] items [\";" + NL + "\t\telse"
			+ NL + "\t\t\tresult = result + \"[TODO] items [\";" + NL + "" + NL + "\t\tresult = result + items;" + NL
			+ "\t\tresult = result + (\"] for [\");" + NL + "\t\tresult = result + (borrowerName);" + NL
			+ "\t\tresult = result + (\"] waiting these borrowers [\");" + NL
			+ "\t\tresult = result + waitedBorrowerNames;" + NL + "\t\tresult = result + \"] from these libraries [\";"
			+ NL + "\t\tresult = result + involvedLibrariesString;" + NL + "\t\tresult = result + \"]\";" + NL
			+ "\t\treturn result;" + NL + "\t}" + NL
