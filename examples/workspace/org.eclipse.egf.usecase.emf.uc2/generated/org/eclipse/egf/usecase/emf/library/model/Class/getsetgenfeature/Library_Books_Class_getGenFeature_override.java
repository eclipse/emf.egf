...
public class Library_Books_Class_getGenFeature_override
	...
	protected final String TEXT_4 = "() " + NL + "    {" + NL + "//INJECTED-CODE-BEGIN : Library.getBooks()" + NL
			+ "        if (books == null) {    " + NL
			+ "            // create a custom list implementation that synchronizes its content" + NL
			+ "            //    with the \"stock\" superset" + NL + "            books = new ";
	protected final String TEXT_5 = "<Book>(Book.class, this, EXTLibraryPackage.LIBRARY__BOOKS) {" + NL
			+ "                private static final long serialVersionUID = 1L;" + NL + "    " + NL
			+ "                @Override" + NL + "                protected void didAdd(int index, Book newObject) {"
			+ NL + "                    if (getStock().indexOf(newObject) == -1) {" + NL
			+ "                        // these lists are unordered, so index doesn't matter" + NL
			+ "                        getStock().add(newObject);" + NL + "                    }" + NL
			+ "                }" + NL + "    " + NL + "                @Override" + NL
			+ "                protected void didRemove(int index, Book oldObject) {" + NL
			+ "                    if (getStock().indexOf(oldObject) != -1) {" + NL
			+ "                        // these lists are unordered, so index doesn't matter" + NL
			+ "                        getStock().remove(oldObject);" + NL + "                    }" + NL
			+ "                }" + NL + "    " + NL + "                @Override" + NL
			+ "                protected void didSet(int index, Book newObject, Book oldObject) {" + NL
			+ "                    didRemove(index, oldObject);" + NL + "                    didAdd(index, newObject);"
			+ NL + "                }" + NL + "            };" + NL + "        }" + NL + "        return books;" + NL
			+ "//INJECTED-CODE-END : Library.getBooks()" + NL + "    }" + NL;
	...
	public boolean preCondition(PatternContext ctx) throws Exception {
		if ("Library".equals(genFeature.getGenClass().getEcoreClass().getName())
				&& "books".equals(genFeature.getName())) {
			return true;
		}
		return false;
	}
