 ...
 */
package org.eclipse.egf.common.ui.toolkit.internal;
...
public class StringMatcher {
  ...
  /**
   * StringMatcher constructor takes in a String object that is a simple pattern which may contain '*' for 0 and many characters and '?' for exactly one
   * character. Literal '*' and '?' characters must be escaped in the pattern e.g., "\*" means literal "*", etc. Escaping any other character (including the
   * escape character itself), just results in that character in the pattern. e.g., "\a" means "a" and "\\" means "\" If invoking the StringMatcher with string
  ...
  public boolean match(String text, int start_p, int end_p) {
    ...
    if (fSegments.length == 1 && fHasLeadingStar == false && fHasTrailingStar == false) {
    ...
    if (fHasTrailingStar == false && tCurPos != end) {
  ...
  /**
   * Parses the given pattern into segments separated by wildcard '*' characters. a String object that is a simple regular expression with '*' and/or '?'
   */
