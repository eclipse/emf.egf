...
/**
 * Open a wizard to create an {@link ContextElement} model element.
 * @author fournier
 */
public class CreateContextElementAction extends CreateModelElementAction {
  /**
   * Constructor.
   * @param shell_p
   ...
   */
  public CreateContextElementAction(Shell shell_p, ISelectionProvider selectionProvider_p, boolean readOnlyMode_p) {
    super(shell_p, selectionProvider_p, readOnlyMode_p);
    setText(Messages.CreateContextElementAction_Title);
    setToolTipText(Messages.CreateContextElementAction_Tooltip);
    setImageDescriptor(EgfWorkbenchActivator.getDefault().getImageDescriptor(IImageKeys.IMG_NEW_CONTEXT_ELEMENT));
  }
  ...
  protected Wizard getWizardFor(NamedModelElementWithId modelElement_p) {
    return new ContextElementWizard(modelElement_p, null, isReadOnly());
  }
