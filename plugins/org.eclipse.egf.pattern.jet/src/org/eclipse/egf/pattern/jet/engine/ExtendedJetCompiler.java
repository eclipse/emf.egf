...
public class ExtendedJetCompiler extends JETCompiler {
	...
	public ExtendedJetCompiler(String templateURI, InputStream inputStream,
			String encoding) throws JETException {
	...
	public void handleDirective(String directive, JETMark start, JETMark stop,
		...
		if (EGF_PATTERN_CALL.equals(directive)) {
            ...
            buffer.append("CallHelper.executeWithInjection(\"" + patternId + "\", new ExecutionContext((InternalPatternContext) ctx), parameters);"); //$NON-NLS-1$ //$NON-NLS-2$
            buffer.append(N);
		    ...
		    buffer.append(");");
            buffer.append(N);
            buffer.append("CallHelper.execute(\"");
	...
	protected void parse(JETCoreElement[] coreElements,
			java.lang.Class<?>[] accept) throws JETException {
