 ...
 */
public class RemovePatternLibraryWrapperCommand extends PluginChangeWrapperCommand {
  /**
   * Constructor.
   * @param domain_p
   ...
   * @param value_p
   */
  public RemovePatternLibraryWrapperCommand(EditingDomain domain_p, EObject modelElement_p, EStructuralFeature feature_p, Object value_p) {
  ...
  protected List<IPluginChangesCommand> getExecutablePluginChangeCommand() {
    // Get the removed pattern library.
    PatternLibrary patternLibrary = (PatternLibrary) getValue();
    // Create a command to change the plug-in file accordingly.
    return Collections.singletonList(PatternExtensionFactory.unsetPatternLibraryExtension(patternLibrary.getId()));
  }
