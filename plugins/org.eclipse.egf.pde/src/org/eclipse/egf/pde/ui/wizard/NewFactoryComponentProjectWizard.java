...
public class NewFactoryComponentProjectWizard extends NewWizard {
   ...
   */
  private static final Logger __logger = Logger.getLogger(NewFactoryComponentProjectWizard.class.getPackage().getName());
  ...
  public NewFactoryComponentProjectWizard() {
    setDefaultPageImageDescriptor(EgfPdeActivator.getDefault().getImageDescriptor(IMG_WIZARD_FACTORY_COMPONENT));
  ...
  private ElementList getAvailableCodegenWizards() {
    ...
    IConfigurationElement[] elements = ExtensionPointHelper.getConfigurationElements(
      EgfPdeActivator.getDefault().getPluginID(), 
  ...
  protected void createMainPage() {
    _mainPage = new NewProjectCreationPage("NewProjectCreationPage", _pluginData, false, getSelection()) { //$NON-NLS-1$
      ...
      public void createControl(Composite parent_p) {
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(getControl(), IHelpContextIds.NEW_FACTORY_COMPONENT_PROJECT_STRUCTURE_WIZARD_PAGE);
      }
    };
  ...
  public boolean performFinish() {
    ...
    try {
      ...
      StringBuilder loggerMessage = new StringBuilder("NewFactoryComponentProjectWizard.performFinish(..) _ "); //$NON-NLS-1$
