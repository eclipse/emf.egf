...
import static org.eclipse.egf.common.constant.ICommonConstants.EMPTY_STRING;
import static org.eclipse.egf.common.constant.ICommonConstants.SLASH_CHARACTER;
...
public abstract class AbstractGenModelGenerator extends AbstractGenerator {
  ...
  private String getModelDirectory() {
    return 
      new StringBuilder(SLASH_CHARACTER)
        .append(_modelPluginId)
        .append(SLASH_CHARACTER)
        .append(_modelDirectory)
        .append(SLASH_CHARACTER)
        .toString();
  }
  ...
  private GenModel createGenModel(Monitor monitor_p) {
    ...
    try {
      ...
      // TODO Stephane : ask EMF newsgroup why the GenModel is invalid ?
      IStatus status = null; // genModel.validate();
      // If the status is OK, let's create the related file disk.
      if (true) {// status.isOK()) {
        try {
        ...
        handleDiagnostic(diagnostic, "Invalid genmodel"); //$NON-NLS-1$
      }
    } catch (Exception exception_p) {
      genModel = null;
    ...
    for (GenPackage genPackage : genPackages) {
      ...
      if (container == null) {
        ...
        while (container != null) {
          suffix = ICommonConstants.DOT_CHARACTER + container.getName() + suffix;
  ...
  protected boolean adjustAttributes(Monitor monitor_p) {
    try {
      ...
      // TODO stephane : faire une seule variable pour stocker tout le genModel.
